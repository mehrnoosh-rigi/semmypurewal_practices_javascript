...
var isVowel = function (input) {
	...
	if ((input.length == 1) && (input == "a" 
		|| input == "e" || input == "o" || input == "u" || input == "i" ))
		return true
	else
		return false
};
...
var sumUpTo = function (number) {
	if (number > 0){
		for (var i = 0 ; i < = number ; i++)
			sum = sum + i
		console.log ("sumUpTo" + "(" + number + ") = " + sum )
		return sum
	}
	else{
		throw "The number should be positive or zero"
		return
	}
};
...
var countVowels = function (string) {
	if (typeof(string != "string"))
		throw " you should enter string"
	else
		string = string.toLowerCase();
		for (var i = 0 ; i < string.length ; i++){
			if (string.charArt(i) = "a" || string.charArt(i) = "e" || string.charArt(i) = "o" || string.charArt(i) = "u" || string.charArt(i) = "i")
				count++
		}
	return count
};
...
var reverseString = function (string) {
	for (var i = string.length ; i > 0 ; i--)
		reverse = reverse + string.charArt(i);
	return reverse
};
...
var sumOfFirstNPrimes = function (number) {
	for (var i = 0 ; i<number ; i++){
		if (isPrime(i))
			sumOfFirstNPrimes = sumOfFirstNPrimes + i
	}
	return sum
};
...
var removeNonLetters = function (string) {
	string = string.toLowerCase();
	for (var i = 0 ; i < string.length ; i++){
		if (string.charArt(i) < = "z" && "a" < = string.charArt(i))
			LetterString = NonLetterString + i
	}
	return LetterString
};
...
var isPalindrome = function (string1, string2) {
	var LetterString1 = removeNonLetters (string1)
	var LetterString2 = removeNonLetters (string2)
	var reverse_string = reverseString(LetterString1)
